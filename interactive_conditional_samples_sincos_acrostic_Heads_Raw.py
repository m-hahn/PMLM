...
#jiant environment on titanxp GPU (tensorflow-gpu==1.13, I think)
...
for group in ["_c"]:
 try:
  with open("/u/scr/mhahn/PRETRAINED/parsing/en_ewt-ud-dev_alternatives"+group+".txt", "r") as inFile:
   for line in inFile:
       if line.startswith("####"):
          next(inFile)
...
count = 0
with open("/u/scr/mhahn/PRETRAINED/parsing/en_ewt-ud-dev_alternatives_PMLM_"+MODEL_NAME.split('/')[-2]+"_raw.tsv", "w") as outFile:
