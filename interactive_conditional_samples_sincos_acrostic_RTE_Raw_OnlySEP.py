 ...
 try:
  with open(f"/u/scr/mhahn/PRETRAINED/GLUE/glue_data/RTE/dev_alternatives{group}.tsv", "r") as inFile:
   for line in inFile:
       ...
       masked = masked[:boundary] + ["▁[SEP]"] + masked[boundary:] # "▁[CLS]"
       #print(masked)
...
count = 0
with open(f"/u/scr/mhahn/PRETRAINED/GLUE/glue_data/RTE/dev_alternatives_PMLM_{MODEL_NAME.split('/')[-2]}_raw_OnlySEP.tsv", "w") as outFile:
