...
for group in [""]:
 try:
  with open("/u/scr/mhahn/PRETRAINED/SyntaxGym/syntaxgym_"+DATASET+"_alternatives.tsv", "r") as inFile:
   for line in inFile:
       if line.startswith("####"):
          next(inFile)
          _ = int(next(inFile))
...
count = 0
with open("/u/scr/mhahn/PRETRAINED/SyntaxGym/syntaxgym_"+DATASET+"_alternatives_PMLM_"+MODEL_NAME.split('/')[-2]+"_raw.tsv", "w") as outFile:
