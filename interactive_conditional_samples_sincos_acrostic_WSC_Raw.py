...
for group in ["_c"]:
 try:
  with open("/u/scr/mhahn/PRETRAINED/WSC/val_alternatives"+group+".txt", "r") as inFile:
   for line in inFile:
       if line.startswith("####"):
          next(inFile)
...
count = 0
with open("/u/scr/mhahn/PRETRAINED/WSC/val_alternatives_PMLM_"+MODEL_NAME.split('/')[-2]+"_raw.tsv", "w") as outFile:
