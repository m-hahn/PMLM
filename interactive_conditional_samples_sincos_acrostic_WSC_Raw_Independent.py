...
for group in ["_c"]:
 try:
  with open("/u/scr/mhahn/PRETRAINED/WSC/val_alternatives"+group+"_OnlySubsetsNoAlternatives.txt", "r") as inFile:
   for line in inFile:
       if line.startswith("####"):
          identifierForDatapoints += 1
          next(inFile)
          referents = [int(x) for x in next(inFile).split(" ")]
          tokenized = next(inFile).strip()
#          print("TOK", tokenized)
          line = next(inFile)
          referents_Original = referents[::]
          if referents[0] > referents[2]:
            referents = referents[2:] + referents[:2]
       if len(line) < 3:
        continue
...
#       assert len(sampled) == len(tokenized.split(" ")), (sampled, tokenized)
       ...
       REFERENT1 = "".join(tokenized.split(" ")[referents[0]:referents[1]]).replace("▁", " ").strip()
       REFERENT2 = "".join(tokenized.split(" ")[referents[2]:referents[3]]).replace("▁", " ").strip()

       maskedPure = masked[::]

       masked.insert(referents[3], "▁[SEP] ")
       masked.insert(referents[2], "▁[SEP] ")
...
##
       ...
       encodedWithMaskPure = demo.encodeInputWithMask(maskedPure, withCaching=True)
       ...
       underscores = [i for i in range(len(encodedWithMask)) if encodedWithMask[i] == 102][:-1] # the last one will come from sentence-final SEP
  #     print([tokenizer.convert_ids_to_tokens(int(x)) for x in masked])
       masked_list_with_underscores = [int(encodedWithMask[i]) for i in range(len(encodedWithMask))]
       masked_list = [masked_list_with_underscores[i] for i in range(len(encodedWithMask)) if i not in underscores]


       assert len(underscores) == 4, underscores
       underscores[3] -= 3
       underscores[2] -= 2
       underscores[1] -= 1
  ...
  #        underscores = underscores[2:] + underscores[:2]
       ...
       REFERENT2_ = (" ".join(BertModel.decodetext([x for x in masked_list[underscores[2]:underscores[3]]], vocab_file="en_vocab.txt",do_lower_case = False))).replace(" ##", "")
       assert REFERENT1.replace(" ", "") == REFERENT1_.replace(" ", ""), (REFERENT1, REFERENT1_)
       assert REFERENT2.replace(" ", "") == REFERENT2_.replace(" ", ""), (REFERENT1, REFERENT1_)

 #      print(tokenized)
      
       tokenized_ = tokenized.split(" ")[::]
       tokenized_.insert(referents[3], "]")
       tokenized_.insert(referents[2], "[")
       tokenized_.insert(referents[1], "_")
       tokenized_.insert(referents[0], "_")
       tokenized_ = " ".join(tokenized_)
#       print(1544, tokenized_)
       #quit()
   ...
   if i+1 == len(blankCandidates):
     ...
     for j in range(i+1, min(len(blankCandidates), i+BATCH_SIZE+1)):
        if not (blankCandidates[j]["PMLM_Mask_Encoded"].startswith(blankCandidates[i]["PMLM_Mask_Encoded"])):
            break
     j -= 1
   if abs(i-j) > 3: # ignore very small batches
     ...
     # print(j-i, i,j, blankCandidates[j]["PMLM_Mask_Encoded"], blankCandidates[i]["PMLM_Mask_Encoded"], (blankCandidates[j]["PMLM_Mask_Encoded"].startswith(blankCandidates[i]["PMLM_Mask_Encoded"])), len(blankCandidates))
      ...
      print("Ignoring small batch", blankCandidates[i])
   i = j+1
print(len(BATCHES))
...
count = 0
with open("/u/scr/mhahn/PRETRAINED/WSC/val_alternatives_PMLM_"+MODEL_NAME.split('/')[-2]+"_raw_Independent.tsv", "w") as outFile:
